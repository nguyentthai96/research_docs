...
|---|---|---|---|
| `--venue=<v>` (v3.2, default) | Venue track | `venue_disclosure_policies.md` database (v2, 15 policy targets: ICLR / NeurIPS / Nature / Science / ACL / EMNLP + the #596 medical-publishing set — ICMJE / NEJM / The Lancet / JAMA / BMJ / PLOS / Frontiers / Chinese Nursing Journals Publishing House / International Eye Science) | Applicability/status bundle: `REQUIRED`, `ACTION_ONLY`, `NOT_REQUIRED`, or `UNKNOWN`, plus an explicit typed halt status when rendering cannot proceed |
...
| Chinese Nursing Journals Publishing House | tool/service name; purpose; affected text/figure/code; human review/editing performed; author acceptance of full responsibility | Use affecting research methods: method-level description |
